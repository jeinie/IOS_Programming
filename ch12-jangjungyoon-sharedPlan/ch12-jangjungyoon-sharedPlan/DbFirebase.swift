//
//  DbFirebase.swift
//  ch12-jangjungyoon-sharedPlan
//
//  Created by 장정윤 on 2023/05/23.
//
...
//        let data = try? NSKeyedArchiver.archivedData(withRootObject: plan, requiringSecureCoding: false)
        var data = [String: Any]()
        data["content"] = plan.content as String
        data["date"] = plan.date
        data["key"] = plan.key as String
        data["kind"] = plan.kind.rawValue as Int
        data["owner"] = plan.owner! as String
...
//            let plan = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(data["data"] as! Data) as? Plan
            ...
            let tempPlan = data["data"] as! [String: Any]
            let kind = tempPlan["kind"] as! Int
            let owner = tempPlan["owner"] as! String
            let key = tempPlan["key"] as! String
            let content = tempPlan["content"] as! String
            let date = tempPlan["date"] as! Timestamp

            let plan = Plan(date: date.dateValue(), owner: owner, kind: Plan.Kind(rawValue: kind)!, content: content, key: key)
